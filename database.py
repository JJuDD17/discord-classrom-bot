...
async def get_task(task_id, description_only=False):
    ...
    if description_only:
        ...
        return tuple(await conn.fetchrow('SELECT file, file_name, description FROM tasks WHERE id = $1', task_id))
...
async def get_solution(task_id, student_id):
    global conn
    return tuple(await conn.fetchrow('SELECT file, file_name FROM solutions WHERE student_id = $1 AND task_id = $2',
                                     student_id, task_id))


async def who_has_solution(task_id):
    global conn
    return [i.student_id for i in await conn.fetch('SELECT student_id FROM solutions WHERE task_id = $1', task_id)]


async def tasks_done(student_id):
    global conn
    return [i.task_id for i in
            await conn.fetch('SELECT task_id FROM solutions WHERE student_id = $1', student_id)]


async def all_task_ids():
    global conn
    return [i.id for i in await conn.fetch('SELECT id FROM tasks')]
...
async def get_mark(task_id, student_id):
    global conn
    result = await conn.fetchval('SELECT mark FROM marks WHERE task_id = $1 AND student_id = $2', task_id, student_id)
    return result


async def get_marks(student_id):
    return [i.mark for i in await conn.fetch('SELECT mark FROM marks WHERE student_id = $1', student_id)]
